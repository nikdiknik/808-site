...
const modalText = {
  tracks: {
    ...
    subtitle:
      "В Free можно вести одну демку и попробовать базовые перезапуски. Premium откроет больше демок в трекере, больше сценариев и расширенные методики",
  },
  restarts: {
    title: "Разблокируй больше перезапусков",
    subtitle: "Premium откроет больше попыток перезапуститься и расширенные методики",
  },
};
      ...
      <div className="w-full max-w-[520px] rounded-[28px] border border-white/8 bg-[#1E1E1E] p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="heading-font mt-3 text-[26px] leading-tight text-white">{text.title}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="mt-5 text-[17px] leading-relaxed text-[#C9C9C9]">{text.subtitle}</p>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <button
            type="button"
            className="flex min-h-[54px] items-center justify-center gap-2 rounded-full bg-[#78F761] px-5 text-[16px] font-bold text-[#0A0A0A] transition hover:brightness-110"
          >
            <Lock size={18} />
            Купить за 119 ₽
          </button>
